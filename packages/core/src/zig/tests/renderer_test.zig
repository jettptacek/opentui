...
test "renderer - setOutputFd redirects output to custom fd" {
    ...
    cli_renderer.setOutputFd(@intCast(@intFromEnum(write_fd)));
...
test "renderer - two renderers write to different fds" {
    ...
    renderer1.setOutputFd(@intCast(@intFromEnum(pipe1[1])));
    renderer2.setOutputFd(@intCast(@intFromEnum(pipe2[1])));
